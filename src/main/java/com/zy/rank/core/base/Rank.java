...
import com.zy.rank.core.node.AbNode;
import com.zy.rank.core.node.Node;
import com.zy.rank.core.node.NodeStepBase;
import com.zy.rank.core.node.SearchAbNodeResult;
import com.zy.rank.core.pool.ILockerPool;
import com.zy.rank.core.pool.LockerPool;
import com.zy.rank.core.pool.RankElementNodeMap;
import com.zy.rank.core.pool.RankPool;
import com.zy.rank.core.reoper.ReOper;
import com.zy.rank.core.reoper.ReOper.OperType;
import com.zy.rank.core.reoper.ReOperService;
...
public class Rank implements IRank {
	...
	public Rank(final RankConfigure rankConfigure) {
		try {
			...
			if (!rankConfigure.check()) {
				throw new RankException("rankConfigure is unavailable!"
						+ rankConfigure);
			}
			...
			head = rankPool.getNode(Long.MAX_VALUE, 0,
					rankConfigure.getRankConditionCount() - 1);
	...
	/**
	 * 当删除在进行的时候，其它操作都不能进行 1、 当删除一个node的时候，不能向其中添加点
	 * */
	@Override
	...
	public long setByField(int id, int field, long value) {
		...
		if (field >= fieldCount) {
			throw new RankException("field too large,field=" + field
					+ ",but rank field is (0-" + (fieldCount - 1) + ")");
		}
		...
		if (oldElement == null) { // 说明原来不存在
			throw new RankException(
					"old data is not exist while try set by field,please set all value before");
		}
		...
		if (!doUpdate(oldElement, element)) {
			if (reOperService.addQueue(element, OperType.Update, 0, null,
					oldElement, null)) {
	...
	private void checkDataBeforeSet(int id, long... value) {
		...
		if (value == null
				|| value.length != rankConfigure.getRankConditionCount()) {
	...
	public List<RankData> getRankDatasByRankNum(int begin, int length) {
		if (begin >= elementMap.size()) {
			log.warn("has no enough player: begin:" + begin
					+ ",elementMapSize:" + elementMap.size());
			return null;
		}
		...
		while (currentNode != null) {
			...
			if (rankNum + countN >= begin) {
				...
				while (elementList.size() < length && currentNode2 != null) {
					currentNode2.getElementsByIndex(elementList, beginE,
							lengthE);
					beginE = 0;
	...
	private RankData doGet(Element element) {
		...
		if (currentNode == null || currentNode.getParentNS() != nodeStep) {
			log.warn("id " + element.getId() + " not exist:currentNode:"
					+ currentNode);
			element.unLock();
			return null;
		}
	...
	private SearchAbNodeResult getStartNodeByValue(long value,
			NodeStepBase nodeStep) {
		int rankNum = 0;
		...
		while (currentNodeStep != null) {
			...
			if (currentNodeStep.getValue() < value) {
				...
				if (previousNodeStep != null
						&& previousNodeStep.getValue() >= value) {
	...
	/**
	 * 这里返回的Node一定是可以作为value对应node的前node，有可能是head {@code SearchNodeStepResult}
	 * */
	private SearchAbNodeResult getStartNodeByNodeStep1(long value) {
		...
		while (currentNodeStepStep != null) {
			if (currentNodeStepStep.getValue() < value) {
				...
				if (previousNodeStepStep != null
						&& previousNodeStepStep.getValue() >= value) {
					currentNodeStep = (NodeStepBase) previousNodeStepStep
							.getHead();
		...
		currentHitTimes = 0;
		Node currentNode = head;
		NodeStepBase previouNodeStep = /*
										 * (NodeStepBase)currentNodeStep.getPrevious
										 * (); //
										 */null;
		while (currentNodeStep != null) {
			if (currentNodeStep.getHead().getValue() < value) {
				...
				if (previouNodeStep != null
						&& previouNodeStep.getHead().getValue() >= value) {
					...
					if (currentHitTimes++ > maxHitTimesNodeStep) {
						log.warn("nodestep:currentHitTimes++>NodeStep.maxHitTimes:"
								+ currentHitTimes);
	...
	/**
	 * 找到位置 1、没有对应的node 锁定上下node 重新校验 创建并添加 解锁 2、存在对应的node add就可以
	 * 
	 * 如何防止重复添加
	 * 
	 * */
		...
		try {// 为了统计循环次数才加的这个，没有其它目的
			while (currentNode != null) {
				...
				if (currentNode.getValue() > value) {
					...
					if (currentNode == null) {
						// 删除的时候需要校验该lock
						Node node = rankPool.getNode(element, value,
								rankConfigure.getRankConditionCount() - 1);
						...
						if (previousNode.getNext() != null
								|| (previousNode.getPrevious() != prePreNode)
								|| (previousNode != head && prePreNode
										.getNext() != previousNode)) {
	...
	private boolean doDeleteNode(Node node) {
		if (node.getCount() <= 0) {
			...
			if ((pre != node.getPrevious() || (pre != null && pre.getNext() != node))
					|| (next != node.getNext() || (next != null && next
							.getPrevious() != node))) {
	...
	private boolean doDelete(Element element) {
		...
		Node node = nodeMap.get(element.getValue()[0]);
		if (node == null) {
	...
	/**
	 * update 实行的是先删除再添加，非原子的，但可以确保数据最终正确 后面可以优化
	 * */
	...
	public boolean doReOper_(ReOper reOper, boolean isLastTime) {
		...
		case Delete:
			...
			if (!success && isLastTime) {
				...
				elementMap
						.put(reOper.getElement().getId(), reOper.getElement());
			}
			break;
		...
		case Update:
			...
			if (!success && isLastTime) {
				reOper.getElement().unLock();
				// 简单放进去，这样并不托，这里发生的时候就是大问题了
				elementMap.put(reOper.getOldElement().getId(),
						reOper.getOldElement());
			}
			break;
		default:
			break;
	...
	private boolean lockMultipleNode(Node... nodes) {
		...
		for (Node node : nodes) {
			...
			boolean isLock = lockerPool.tryLockNodeWLocker(node,
					rankConfigure.getRankConditionCount() - 1);
			if (!isLock) {
				for (int j = nodes.length - 1; j >= 0; j--) {
					if (lockNodes[j] != null) {
						lockerPool.unlockNodeWLocker(lockNodes[j],
								rankConfigure.getRankConditionCount() - 1);
					}
					// 这里
				}
	...
	/**
	 * 在此之前的都处理完结束 如果这个函数花费时间很长，说明访问数量过度，很可能是对较少的id作了较大的访问
	 * */
	@Override
	...
	private void unLockMultipleNode(Node... nodes) {
		for (Node node : nodes) {
			...
			lockerPool.unlockNodeWLocker(node,
					rankConfigure.getRankConditionCount() - 1);
		}
	}
