...
import com.zy.rank.core.node.RankElement;
import com.zy.rank.core.node.RankElementNode;
...
public class RankStatistics {
	 ...
	 * 包括统计的数据和校验使用的统计数据
	 * 
	 * Element的总数 Node的总数 NodeStep的总数 NodeStepStep的总数
	 * 
	 * Element加锁次数 Node加锁次数 Node中ElementStep合并次数(没有拆分) NodeStep拆分和合并次数
	 * NodeStepStep的拆分和合并次数
	 * 
	 * 
	 * ReOper总次数 ReOper栈中的未执行任务reOperQueue
	 * 
	 * 执行的set次数 执行的get次数 执行的delete次数 以上三者的执行频率，效率等 定位难度（平均即每个访问需要花费的定位访问次数）
	 * 
	 * 
	 * 
	 * 校验相关： 当前还没有重做的任务数reOperTaskCount：验证
	 * Element的总数==Node中的Element和==NodeStep中的Element和==NodeStepStep中的Element总数和
	 * Node的总数==NodeStep中的Node总数 NodeStep的总数==NodeStepStep中的NodeStep总数
	 * 
	 * 拆分失败
	 * 
	 * 大小关系 效率验证 配置状态
	...
	public int testCycleNode(Object object, int p_count/* 从父node中计算的count */) { // 这是一个比较好的校验函数
		...
		while (cu != null) {
			if (cu.getConditionLevel() > 0 && cu.getValue() < Long.MAX_VALUE) {
				count += testCycleNode(((RankElementNode) cu).getRankElement(),
						cu.getCount());
			}
			pre = cu;
			cu = (Node) cu.getNext();
		...
		if (object instanceof Rank) {
			...
			if (r.getNodeCount() != count2) {
				System.err.println("rank:r.getNodeCount():" + r.getNodeCount()
						+ ",count2:" + count2);
			}
		} else {
			RankElement r = (RankElement) object;
			if (r.getNodeCount() != count2 - 1/* || r.getNodeCount() != p_count */) { // r.getNodeCount()没有计算head
				System.err.println("rankelement:r.getNodeCount():"
						+ r.getNodeCount() + ",count2:" + count2 + ",p_count:"
						+ p_count);
			}
		}
		return count;
	...
	public String getNodeAndStepCount() {
		...
		// NodeStepStep
		List<NodeStepBase> nodeStepStepList = getNodeStepStepList(rank
				.getHead().getParentNS().getParentNS());
		...
		int countPerDoAdd = searchNodeCycCountPer.get()
				/ countSearchNodeCountPer;
		...
		sb.append("SearchNodeCycCountPer(difficulty of location):"
				+ countPerDoAdd + ",searchNodeCountPer:"
				+ countSearchNodeCountPer + "\n");
		return sb.toString();
	}
	...
	public String toString() {
		...
		List<NodeStepBase> nodeStepList = getNodeStepList(rank.getHead()
				.getParentNS());
		...
		for (NodeStepBase nodeStepStep : nodeStepStepList) {
			...
			System.err.println("---------------------------------:"
					+ nodeStepStep.getNodeCount() + ","
					+ nodeStepStep.getElementCount());
		}
		for (NodeStepBase nodeStep : nodeStepList) {
		...
		sb.append("nodeStepCount:" + nodeStepCount1 + "," + nodeStepCount2
				+ "\n");
		sb.append("nodeCount:" + nodeCount1 + "," + nodeCount2 + ","
				+ nodeCount3 + "\n");
		sb.append("elementCount:" + elementCount1 + "," + elementCount2 + ","
				+ elementCount3 + "," + elementCount4 + "\n");

		sb.append("setCount:" + setCount.get() + ",getCount:" + getCount.get()
				+ ",deleteCount:" + deleteCount.get() + "\n");
		sb.append("reOperQueueSize:" + rank.getReOperQueueSize() + "\n");

		sb.append("nodeLockTimes:[" + getNodeLockCount() + ","
				+ getNodeUnlockCount() + "],elementLockTimes:["
				+ getElementLockCount() + "," + getElementUnlockCount() + "]"
				+ "\n");
		sb.append("elementStepCombineTime:" + elementStepCombineTime.get()
				+ "\n");
		sb.append("combineNodeStepCount:" + combineNodeStepCount.get()
				+ ",cutNodeStepCount:" + cutNodeStepCount.get() + "\n");
		sb.append("combineNodeStepStepCount:" + combineNodeStepStepCount.get()
				+ ",cutNodeStepStepCount:" + cutNodeStepStepCount.get() + "\n");
	...
	private void showStackStr(
			final ConcurrentHashMap<Thread, StackTraceElement[]> elementLockStackTraceMap) {
		for (Entry<Thread, StackTraceElement[]> entry : elementLockStackTraceMap
				.entrySet()) {
	...
	private List<NodeStepBase> getNodeStepListByNodeStepStep(
			NodeStepBase nodeStepStep) {
		List<NodeStepBase> result = new ArrayList<NodeStepBase>();
		...
		if (currentNodeStep == null && i != nodeStepCount) {
			log.error("currentNodeStep == null,i:" + i + ",nodeStepCount:"
					+ nodeStepCount);
		}
		return result;
	}
	...
	private List<Node> getNodeListByNodeStep(NodeStepBase nodeStep) {
		...
		for (; i < nodeCount && currentNode != null; i++) {
			...
			if (currentNode.getParentNS() != nodeStep) {
				...
				log.error("currentNode.getNodeStep() != nodeStep:"
						+ currentNode.getParentNS() + "!=" + nodeStep);
			}
